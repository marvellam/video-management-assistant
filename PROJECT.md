...
- `template.json` 是只读的官方推荐模板；用户可以在软件内从官方结构复制，或从空白开始建立自定义模板。
- 自定义模板保存在当前电脑的应用配置目录；图形预览和实际生成共用同一套 Rust 生成核心。
- 项目根目录继续固定为 `日期_项目名称`，自定义范围只开放其下的目录树。
...
- v1.2.0 本地试用版加入软件内目录树设计器：官方模板只读，自定义模板支持复制官方、空白新建、改名、添加同级/子目录、删除节点、上移/下移、保存与删除模板。
